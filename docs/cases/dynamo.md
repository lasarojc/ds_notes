...
DynamoDB é um marco nos bancos de dados NoSQL. 
...
Ao assistir ao vídeo, alguns pontos devem ser ressaltados sobre o DynamoDB especificamente e os bancos NoSQL de forma geral:
...
* maior escalabilidade implica maior exposição a particionamentos da rede em que o sistema roda, que associado à necessidade de manutenção de alta disponibilidade, implica perda de garantias de consistência (veremos o [Teorema CAP](https://en.wikipedia.org/wiki/CAP_theorem) adiante);
...
Nesta aplicação, cada usuário tem um **identificador único**, recuperado no momento em que faz login no sistema da Amazon.
