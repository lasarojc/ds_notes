...
durante a remoção de um nó, garantindo que a estrutura da rede não se desfaça.
...
roteamento que é a estrutura chave para o algoritmo de roteamento.
...
!!!example "Algoritmo de roteamento"
    ...
    * $L_i$ o i-ésimo $id$ do conjunto de folhas $L$, $-[\frac{|L|}{2}] \leq i \leq [\frac{|L|}{2} ]$, onde índices positivos e negativos indicam $id$s menores ou maiores que o $id$ $A$, respectivamente.
    * $D_{l}$ o valor do l-ésimo dígito da chave $D$
...
!!!example "Procedimento para inserir um novo nó"
    ...
    * gera-se o $id$ através do cálculo do hash SHA-1 sobre o seu endereço ip ou com a sua chave pública

    * Pastry Network passa a rotear uma mensagem especial _join_, onde assim como qualquer mensagem, Pastry busca encontrar o nó mais próximo numericamente do $id$, que no caso é o $id$ do novo nó
    ...
    * assumindo que nó $A$ é o mais próximo de $X$, então inicializamos o conjunto de vizinhos de $X$ com os vizinhos de $A$
...
Perceba que a inserção utiliza do algoritmo de roteamento, e apesar de ao final ele enviar uma cópia do seu estado para todos os nós, são todos os nós que ele tem alguma proximidade e não todos os nós da rede e essa quantidade de nós é um parâmetro
...
Em um dado momento nós podem pedir para sair ou falharem sem nenhum tipo de aviso. A rede Pastry considera que um nó falhou no momento que os nós vizinhos não conseguem mais se comunicar com esse nó. Dado um nó $A$ que possui um leaf set $L$, caso esse nó precise repor um nó do leaf set que falhou. então o nó $A$ contacta o nó de $L$ com o maior index possível, seja ele positivo ou negativo, por exemplo se o nó $L_i$, dado que $[\frac{|L|}{2}] < i < 0$. então o nó $A$ irá enviar uma mensagem para o nó $L_{-[\frac{|L|}{2}]}$, pedindo o seu conjunto leaf set $L^{'}$, depois o nó $A$ escolhe um nó que não pertence ao seu conjunto $L$ e o adiciona, lembrando que $A$ remove o nó falho do conjunto $L$. Perceba que o conjunto de vizinhos (neighborhood set) de um nó exerce um papel importante de ficar em constante troca de mensagens, para verificar se um dado nó ainda está vivo.

### Atualização da tabela de rotas de um nó

Perceba que durante a saída de um nó a tabela de roteamento não é modificada, isso se
dá porque Pastry adota uma solução em que essa atualização ocorre durante o roteamento. Durante o roteamento um dado nó $A$ pode conter uma célula vazia, ou seja,
...
Um ponto importante no algoritmo de inserção é assumir que conhecemos o nó mais
próximo a partir de alguma métrica de distância, então se analisarmos o fato que
...
de $X$, mas e a linha 1? Bom, se olharmos o pior caso que é sendo $A$ não tendo
...
ele recebeu os nós que estão a um nó mais próximo dele, da mesma forma que quando $B$ foi criado, ele recebeu os nós a **1 nível** de distância dele, o mesmo se
aplica a $X_3$ e $C_3$ e assim por diante até $X$ receber o leaf set $Z$, podemos
