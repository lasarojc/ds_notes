...
No exemplo seguinte, o resultado é o mesmo que no exemplo anterior, sem falhas.
...
No Exemplo, entre as entregas da "Versão B" e "Versão A" para o servidor no centro do diagrama, vale a "Versão B".
Também pelo gráfico, vemos que a "Versão B" foi criada **depois** da "Versão A", mas que a versão final vista pelo servidor é exatamente a "A". 
Logo, questionamos se esta abordagem é correta.
Afinal ordem de chegada ao servidor não serve como critério para escolha, afinal, a ordem de chegada dos arquivos ao servidor não reflete necessariamente a ordem em que os arquivos foram criados.
