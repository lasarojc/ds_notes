...
Se você se lembrar da pilha de protocolos de comunicação de referência OSI, lembrará que há uma organização em camadas em que cada camada é responsável pela comunicação em um nível de abstração que serve de base para o nível imediatamente superior.
...
Assim, embora tenhamos a impressão de que cada camada conversa com a equivalente do outro lado da comunicação, na prática, a comunicação desce e sobe a pilha.
...
O IP trabalha conectando diversas redes por meio de **roteadores**, nós que fazem parte de mais de uma rede.
Nestes nós, os pacotes de dados sobem somente até a camada 3 e lá são encaminhados por **melhor esforço** para o próximo segmento rumo ao destinatário.
...
Como usuários da pilha IP, temos que entender que o fato de a pilha IP ser mais simples que a OSI implica que as funcionalidades das camadas 5 e 6 devem ser implementadas na camada de aplicação, por você, desenvolvedor.
Contudo, não tema! Estas funcionalidades podem ser normalmente implementadas por meio de *frameworks* ou do *middleware* em uso.
...
Esta característica é conhecida como o [argumento fim-a-fim no projeto de sistemas](http://web.mit.edu/Saltzer/www/publications/endtoend/endtoend.pdf); uma análise recente deste argumento foi feita [aqui](https://blog.acolyer.org/2014/11/14/end-to-end-arguments-in-system-design/).
